<template>
  <div class="paging">
    <button type="button" class="btnPrev" :class="{ disabled: current === 1 }" @click="prevPage">
      이전 페이지
    </button>
    <button
      v-for="(props, i) in props?.totalPage"
      :key="i"
      :class="i + 1 == current ? 'accent' : ''"
      @click="returnCurrent(i)"
    >
      {{ i + 1 }}
    </button>
    <button
      type="button"
      class="btnNext"
      :class="{ disabled: current == props?.totalPage }"
      @click="nextPage"
    >
      다음 페이지
    </button>
