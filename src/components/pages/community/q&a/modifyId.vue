    ...
    <div class="contents">
      ...
      <div class="w943">
        ...
        <div class="inputTable qna">
          <table>
            <caption>문의유형, 제목, 문의내용, 사진, 비밀글 비밀번호 항목으로 구성된 문의하기 입력 표</caption>
            <colgroup>
              <col style="width:14%">
              <col style="width:36%">
              <col style="width:14%">
                ...
                >
                  ...
                  >
                    <ul v-if="datas?.beforeImg.length > 0">
                      <li
                          class="pos-rel"
                          v-for="(item, idx) in datas.beforeImg"
                          :key="idx"
                      >
                        <img
                            :src="`http://localhost:8080/files/${item}`"
                            alt="이전에 등록된 이미지"
                        />
                        <button
                            class="delete"
                            @click="removeItem(idx, 'old')"
                        >
...
const checkedId = computed(() => {
  return qnaContent?.value?.id === Number(route.params.id)
          ? qnaContent?.value?.id
            : null;
});
